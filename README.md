...
Theo PLAN §11/§13: **contract + simulator → vertical slice → fairness → recovery → Web UI → nghiệm thu/release**. Contract `1.0.0-b01` đã đóng R-03, R-05 và R-09 qua focused rereview cùng verification mới; ACC-01 là `pass`. B01–B11 đã được duyệt. Chặng tiếp theo là **B12 — CLI** cho upload/submit/status/events/download/admin; B13 và B14 cũng đã đủ dependency B11 và có thể triển khai song song theo PLAN. Evidence runtime B10/B11 hiện giới hạn ở Docker Desktop Linux VM; các gate bare-Linux/portability/release vẫn thuộc các chặng sau. B23 GPU có điều kiện; thiếu GPU không chặn lõi CPU nhưng chặn claim GPU verified.
