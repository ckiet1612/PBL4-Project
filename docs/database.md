...
| `RetrySchedule` | P `retry_schedules(job_id,retry_number)` | Bounded retry/jitter checks; partial ready index | INV-08/22; migration/index tests | **B14 implemented (minimal):** B11 cleanup keeps classifying the attempt failure at release-after-proof; an `INFRASTRUCTURE` failure with budget left (`retry_count < max_retries`), desired `RUNNING` and a restorable Job (a `COMMITTED` non-corrupt checkpoint or template `restart_safe`) opens the schedule (backoff `min(30, 2^(n-1))` s plus 0-1000 ms jitter), increments `retry_count` and sets `jobs.retry_ready_at`; the leader coordinator promotes due schedules to `QUEUED` with CAS on state/version (`RETRY_READY/BACKOFF_ELAPSED`) or leaves them `RETRY_WAIT` with `RETRY_BLOCKED`. B15 owns lease-expiry/reaper retries, pause/cancel interaction and the full recovery matrix |
...
| `CheckpointReservation` | P `checkpoint_reservations(checkpoint_id)` | Unique job sequence across attempts and callback; one `RESERVED` per attempt; exact authority FK; deferred final-state check keeps a recognized checkpoint source `COMMITTED` | INV-11/14/15; cross-attempt sequence, uniqueness, uncommitted-recognition and post-recognition mutation tests | **B14 implemented:** fenced reserve takes `jobs.checkpoint_sequence + 1` in the same transaction and replays the stored pair; publish commits `COMMITTED`, a deterministic validation error sets `REJECTED`, failure/cleanup sets an open row `ABANDONED`; `REJECTED`/`ABANDONED` never become restorable and their sequence is never reused |
| `Checkpoint` | F+P `checkpoints(checkpoint_id)` | Immutable recognized row; unique `(job,sequence)`; exact reservation/attempt/artifact provenance; manifest checksum must equal the committed artifact checksum; job index | INV-11/14/15; checksum/provenance, committed positive and reserved negative tests | **B14 implemented:** publish validates the canonical closed manifest out of transaction, then inserts the row with owner `CHECKPOINT` artifact references; claim verifies candidates newest-first (blob checksum, manifest identity, provenance, compatibility), falls back to the previous one and records the choice immutably in `attempts.execution_context.restore_checkpoint`. No prune path exists, so every committed checkpoint (>=2) stays referenced; operational GC is B19 |
| `CheckpointReference` | P `checkpoint_references(target_job_id,source_checkpoint_id)` | Same-tenant job/checkpoint composite FKs | INV-01/15/16; metadata/migration parity | Not written by B14: automatic recovery restores only its own Job's checkpoints. Manual-retry inheritance through this reference is B15; GC protection is B19 |
| `CheckpointCorruption` | P `checkpoint_corruptions(checkpoint_id)` (migration `20260926_0018`, `schema_v15`) | Insert-only; same-tenant FK to `checkpoints`; closed reason code; `BEFORE UPDATE OR DELETE` rejects mutation | INV-11/15; migration upgrade/downgrade, trigger and FK tests | **B14 implemented:** restore marks a checkpoint whose bytes or manifest fail verification; the API derives `state = CORRUPT` because `checkpoints` itself stays immutable `COMMITTED` |
...
- **B14 implemented (awaiting Task Review):** fenced checkpoint reserve/publish,
  provenance/compatibility validation, insert-only corruption marking, newest-first restore
  with fallback, minimal retry promotion and retention >=2 without a prune path. GC
  protection remains B19.
