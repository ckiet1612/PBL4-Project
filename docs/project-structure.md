...
Dẫn xuất từ [PLAN.md](../PLAN.md) §3–§5, §7–§11, §13–§14; **PLAN được ưu tiên nếu có mâu thuẫn**. Trách nhiệm module đã được khóa; cây thư mục cấp boundary đã được dựng bằng marker theo yêu cầu scaffold được duyệt. B01–B10 đã được Task Review duyệt theo các xác nhận hiện có. B11 có implementation và evidence cục bộ chưa commit, đang chờ review độc lập R01–R16; các gate Linux-host/runtime còn lại không được suy ra từ evidence phát triển.

## Repository structure baseline và B11 additions

Cây file nền sau khi B09 được implement (không liệt kê metadata `.git/` và generated/cache đã ignore):
...
B10 bổ sung worker agent, `schema_v4.py` và migration `20260921_0004`. B11 working tree bổ sung `src/nexa/coordinator/{service,snapshot,dispatch,accounting,runtime,main}.py`, các service `execution_*`/`result_validation`, API worker/result, worker dispatch/result flow, `schema_v5.py`/`schema_v6.py`, migrations `20260922_0005`–`20260923_0007`, tests và [evidence B11](evidence/B11-coordinator-dispatch-result.md). Mục trên là baseline để dễ đọc; các file B10/B11 mới được mô tả tại đây.

B13 mở rộng coordinator/scheduler hiện có, không thêm service scheduler thứ hai. `schema_v7.py`–`schema_v14.py` và migrations `20260925_0008`–`20260925_0017` giữ queue head, tập submitter queued, mốc mở khóa concurrency, mốc retry-ready, event eligibility theo mutation, mốc tắt/bật tenant, index age probe theo submitter, bậc headroom quota theo tenant (`quota_headroom_steps`) và histogram kích thước Job queued (`queue_request_sizes`). Cột chữ ký reconciliation là di sản, không còn điều khiển quét queue mỗi tick. Trigger cập nhật head/submitter/histogram trong transaction Job; allocation chỉ cập nhật bậc headroom quota của tenant, không ghi Job hay event; event chỉ ghi khi policy capacity, worker inventory hoặc trạng thái bật tenant đổi tập Job khả thi. `coordinator/eligibility.py` replay tối đa 64 Job mỗi tick và chặn dispatch của tenant còn event pending. Snapshot chỉ dùng dữ liệu dẫn xuất để đề xuất, còn commit revalidate nguồn sự thật. `coordinator/runtime.py` chạy thêm luồng `coordinator-accounting` gọi `CoordinatorService.account` tối đa mỗi 250 ms trong transaction riêng, chỉ khóa ledger, nên cadence ledger không phụ thuộc độ dài tick quyết định; lỗi DB dừng vòng coordinator (fail closed). `benchmarks/b13/` chứa queue microbenchmark, HTTP API prefill, phép đo cadence ledger commit và runtime weighted fairness, tách lớp bằng chứng; `tests/integration/test_coordinator_b13.py` kiểm head, cursor, aging, accounting batching và stale proposal. Trạng thái gate và giới hạn đo nằm tại [B13 evidence](evidence/B13-production-fairness.md).

B14 bổ sung checkpoint/restore CPU trên các boundary sẵn có, không thêm process mới. Server: `application/checkpoint_{service,validation,restore}.py` (reserve/publish có fence, manifest canonical đóng, chọn restore mới nhất trước và fallback), `coordinator/retry.py` (promotion tối thiểu), `schema_v15.py` + migration `20260926_0018` (bảng insert-only `checkpoint_corruptions`), route `GET /v1/jobs/{job_id}/checkpoints` và lệnh CLI `nexa job checkpoints`. Worker: `worker/checkpoint_flow.py` ghi journal từng bước chu kỳ checkpoint, `dispatch.py`/`executor.py` mount restore chỉ đọc, `execution.py`/`agent.py` phát hiện container workload đã thoát. Workload: `workloads/cpu_state.py` (snapshot canonical), adapter/entrypoint resume từ bước k và runner xử lý protocol checkpoint. Image CPU mới gắn nhãn `io.nexa.runner.checkpoint=cpu-state-v1`. Trạng thái gate và giới hạn nằm tại [B14 evidence](evidence/B14-cpu-checkpoint-restore.md).
...
**Các directory product boundary dưới đây bắt đầu từ scaffold; hiện `domain`/`scheduler` có policy B04, `infrastructure/persistence` có schema B05/B06, `application`/`api`/maintenance CLI có implementation B06–B08, và worker/workloads có các primitive B09.** B02 đặt bootstrap package/config ở `src/nexa/` root và static shell trong `web/`; coordinator B11 đã có process riêng trong working tree; product CLI B12 vẫn chỉ giữ marker. `docs/` tiếp tục giữ vai trò tài liệu; hai project skills được liệt kê ở phần baseline phía trên. PLAN quy định module/đầu ra, không quy định tên từng Python package. Thay tên đường dẫn được cập nhật tại đây; thay boundary/stack cần duyệt theo [ADR](adr.md).
...
| `src/nexa/application/` | B06 identity/admin/policy, B07 artifact, B08 job, B10 worker authority và B11 execution/result/cleanup service; authorization, idempotency/JCS và transaction orchestration | Domain/policy và các interface; dùng adapter persistence/artifact qua composition của process | B06–B10, B11, B14–B15 |
| `src/nexa/api/` | B06 FastAPI app factory, B07 artifact, B08 job/session/event, B10 worker REST và B11 offer/result/input operations, strict JSON/credential/CSRF handling và safe error mapping | Application/domain, persistence/artifact adapter được wire trong lifespan; không auto-migrate, seed principal hay truy cập Docker socket | B06–B10, B11, B15 |
| `src/nexa/coordinator/` | B11 leadership, B04 snapshot/accounting tick và allocation/dispatch; B14 chỉ thêm promotion tối thiểu `RETRY_WAIT → QUEUED` khi backoff bền vững đã hết (`retry.py`); reaper/lease-expiry/recovery thuộc B15 | Scheduler/application + persistence; recheck dưới transaction; không điều khiển Docker trực tiếp | B11, B13, B15 |
...
| `migrations/` | Alembic schema/version/index/constraint | Một head `20260923_0007`: B05–B10 revisions bất biến; B11 thêm dispatch/claim, upload grant lineage và completion callback identity; không auto-run lúc import/request | B05–B11, B21, B25 |
...
| `.env.example`, `compose.yaml` | Config mẫu và Compose | `compose.yaml` có thêm process coordinator B11 không dùng Docker socket; cú pháp config pass với biến bắt buộc. Secret bằng file, DB URL cho API/coordinator, Docker socket chỉ cho worker; worker state dùng host bind cùng absolute path để daemon thấy input/launch spec, không tự chuyển volume B10 cũ; B11 Docker E2E chưa chạy. Đây chưa phải deployment acceptance B21. | B02, B09–B11, B21, B25 |
...
| B10 scoped source/evidence | `src/nexa/application/worker_service.py`, `src/nexa/api/routes_worker.py`, `src/nexa/infrastructure/persistence/schema_v4.py`, migration `20260921_0004`, `src/nexa/worker/{agent,client,credentials,main,runner_control,singleton,state}.py`, `deploy/b10/`, `compose.yaml`, `docs/worker-agent.md`, `docs/evidence/B10-worker-heartbeat-reconcile.md` | Real API/PostgreSQL authority/heartbeat plus nonempty worker reconciliation, exact adoption/renewal, runner deadline/control and journal-backed cleanup. B10 was independently approved per the user's confirmation; its local/Docker Desktop evidence does not establish bare Linux, release deployment or later B11/B15 flows. |
| B11 working-tree source/evidence | `src/nexa/coordinator/`, `src/nexa/application/{execution_service,execution_cleanup,execution_artifacts,result_validation}.py`, `src/nexa/worker/{dispatch,execution,result_flow}.py`, `src/nexa/infrastructure/persistence/schema_v5.py`/`schema_v6.py`, migrations `20260922_0005`–`20260923_0007`, B11 tests, `docs/coordinator.md`, `docs/evidence/B11-coordinator-dispatch-result.md` | HTTP/PostgreSQL focused cases, two-tenant real CPU Docker slice, pre-cleanup worker restart and snapshot/dispatch interleaving passed locally; independent review and B15 old-claim recovery remain open. |
