...
| `src/nexa/application/` | B06 identity/admin/policy, B07 artifact, B08 job và B10 worker API authority/heartbeat/reconciliation service; authorization, idempotency/JCS và transaction orchestration | Domain/policy và các interface; dùng adapter persistence/artifact qua composition của process | B06–B10, B11, B14–B15 |
| `src/nexa/api/` | B06 FastAPI app factory, B07 artifact, B08 job/session/event và B10 worker REST operations, strict JSON/credential/CSRF handling và safe error mapping | Application/domain, persistence/artifact adapter được wire trong lifespan; không auto-migrate, seed principal hay truy cập Docker socket | B06–B10, B11, B15 |
...
| `src/nexa/worker/` | B09 resource discovery/probe, Docker executor/journal; B10 thêm singleton, credential, durable callback state, heartbeat/poll, nonempty reconciliation, exact adoption/renewal và runner deadline/control | API client cho control plane; `ResourceProvider` và `Executor`; Docker socket chỉ tại worker; không query DB trực tiếp | B09–B10, B23 |
...
| `migrations/` | Alembic schema/version/index/constraint | Một head `20260921_0004`: B05/B06/B07 revisions bất biến, B10 thêm progress phân trang reconciliation trên incarnation; không auto-run lúc import/request | B05–B10, B21, B25 |
...
| `.env.example`, `compose.yaml` | Config mẫu và Compose | `compose.yaml` B10 là hợp đồng dev single-host; đã kiểm tra syntax và có smoke restart trong Docker Desktop Linux VM. Secret bằng file, DB URL chỉ cho API, Docker socket chỉ cho worker. Đây chưa phải deployment acceptance B21. | B02, B09–B10, B21, B25 |
