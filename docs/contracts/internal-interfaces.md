...
| poll | worker + incarnation | READY+ENABLED; return ≤1 committed dispatch offer whose lease is unrevoked and live at PostgreSQL DB time (`lease_expires_at > now`), or no-work | Response loss: poll again; expired/revoked leases are never offered; offer alone never authorizes execution |
...
| fail | Authority + typed observation + callback | Exact live authority; observation may contain container identity or `NoContainerProof`; commit failure/fence/quarantine. For a CLAIMED attempt whose start was rejected after Docker create, atomically bind its first observed exact container identity with failure so stopped proof can be checked later | Replay returns original ack. Pre-create failure writes executor tombstone before proof, so delayed start is rejected locally and by stale API authority. A rejected late start stops its exact container and waits for proof-based cleanup; no startup budget extension or capacity release on observation alone |
...
The fetch in step 3 is `GET /workers/{worker_id}/reconciliation`, a signed-cursor page bound to worker identity/incarnation, maximum 100 rows/page. It returns page `server_time` and every unreleased Allocation known to the server for that worker with current/revoked Authority, `lease_expires_at`, desired job state, allocation state, dispatch-committed `startup_nonce`, derived `claim_state`, expected container identity when known, and a nullable committed completion receipt tied to the recognized Result. Worker drains all pages and performs required adopt/stop/cleanup before asserting `reconcile_complete=true`; a cursor/snapshot conflict restarts the bounded scan. A current-incarnation `UNCLAIMED` allocation with no local execution or container remains eligible for the first poll; a pending claim uses its original callback only after exact journal/Docker reconciliation, even while READY is blocked. A succeeded attempt's receipt can repair a lost `/complete` response across incarnation change only when its callback ID, reserved Result ID and committed manifest Artifact binding match the durable local result flow; otherwise it remains unresolved.
