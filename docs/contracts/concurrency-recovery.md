...
1. Existing idempotency scope row (or unique insert/savepoint for a new key); see the worker-callback exception below.
...
Worker callbacks lock the current global policy row before inserting or locking their callback receipt: the receipt's worker foreign key acquires a worker-key lock. Fail and cleanup callbacks that subsequently update policy/counters take `FOR UPDATE` on that row from the outset; other callbacks use their required policy lock before the receipt as well. They then acquire any remaining policy/counter, worker, Job, Attempt and allocation locks in the listed order. Other idempotency operations retain step 1.
...
| Cleanup/release | allocation RELEASED/ledger close/counter changes/job continuation/event/callback commit | cleanup identity, proof union, startup/container rows, release-once guard | Exact-once release and subsequent recovery transition; a pending `202` callback receipt advances to `200` in the release transaction | Inspection/tombstone occurs before; mismatch or create-in-flight returns without release; identical pending replay retains its original `202` until release |
...
- Start budget expires after Docker create but before the API start callback: the rejected start does not extend the 30-second budget or record a running attempt. The worker reports the exact locally bound container identity while its claim authority is still live; the failure transaction binds that first observed identity, revokes/fences and quarantines. The worker stops that identity under its attempt lock, then cleanup verifies its stopped proof against the binding before release. If failure or cleanup cannot commit, it stops conservatively and retains the pending callback/allocation for reconciliation; a 202 is not release.
